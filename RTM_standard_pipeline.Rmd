...
```{r data profiling, echo=TRUE, message=FALSE, warning=FALSE}
...
# the graph should show that the data was well distributed over time
...
group_by( ADMIN0Name, EnuName) %>%
summarise(Completed_Count = n() , avg_staples = round (mean(FCSStap),1) , avg_pulses = round (mean(FCSPulse),1) , avg_fruits = round (mean(FCSFruit),1) , avg_proteins = round (mean(FCSPr),1) , avg_veg = round (mean(FCSVeg),1) , avg_dairy = round (mean(FCSDairy),1) , avg_sug = round(mean(FCSSugar),1) , avg_fat = round(mean(FCSFat),1))

kableExtra::kable(Enum_completed_surveys , format = 'markdown')

Enum_completed_surveys$EnuName [duplicated(Enum_completed_surveys$EnuName)]
