...
write.csv(res_all , 'Syria_mvam_May_analysis.csv')


#upload the results in sql database (Master_mVAMStats table)
del_query <- sprintf(paste ("DELETE from mvamStats Where SvyID = " , SvyID))
sqlQuery(connection, del_query)
sqlSave(channel =  connection , dat= res_all , tablename = 'Master_mVAMStats' , append = T, rownames =  F )
